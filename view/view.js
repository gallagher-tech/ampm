var View = Backbone.View.extend({
  ...
  initialize: function () {
    ...
    // --- Added: listen for live log broadcasts from server ---
    this._socket.on(
      "log",
      _.bind(function (data) {
        //if server is already sending a timestamp, use it. Otherwise, generate new timestamp.
        const timestamp = data.time || new Date().toISOString();

        // format log line similar to server output
        const logLine = `${timestamp} ${data.level}: ${data.message}`;

        // print to browser console.
        console.log(logLine);

        // append to web UI log list
